...
export class EventFormComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private dp: DatePipe, private http: HttpClient,
    private eventService: EventService, private activatedRoute: ActivatedRoute, private placeService: PlaceService) {
  ...
  create() {
    ...
    this.eventForm.get('event').get('end').setValue(this.dp.transform(new Date(this.eventForm.get('event').get('end').value),
      'dd/MM/yyyy hh:mm'));
    this.eventForm.get('event').get('conference').setValue('5cd056bccf7bd600041c2b30');
